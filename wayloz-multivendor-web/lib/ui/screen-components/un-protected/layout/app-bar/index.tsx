...
const AppTopbar = ({ handleModalToggle }: IAppBarProps) => {
      ...
      return (
        <div className="p-3">
          <div className="flex flex-row justify-between">
            ...
            <span
              className="text-sm font-normal mb-2 text-sky-500 hover:cursor-pointer dark:text-primary-color"
              onClick={() => {
                deleteSearchedKeywords();
                setSearchedKeywords([]);
              }}
  ...
  const model = languageTypes.map((lang) => ({
    ...
    template(item: any) {
      return (
        <div
          className={`hover:bg-primary-color ${currentLocale === lang.code ? "bg-primary-color text-white" : ""} p-2 cursor-pointer`}
