    ...
    <div className="bg-white md:max-w-md w-100 w-full relative dark:bg-gray-800 dark:text-gray-200">
      ...
      <div className="py-3 px-6 mb-4">
        ...
        <div className="flex items-center justify-between gap-x-2 mt-4">
          ...
          <button
            className={`${isFormValid() ? "bg-primary-color" : "bg-gray-300 dark:bg-gray-400"} text-white px-4 py-2 text-[500] font-[14px] rounded-full flex flex-col md:flex-row items-center justify-between flex-[0.8]`}
            onClick={handleAddToCart}
            disabled={!isFormValid()}
            type="button"
          >
            {t("add_to_order")}
            <span className="ml-2  text-white font-[14px]">
              {CURRENCY_SYMBOL}
              {calculateTotalPrice()}
            </span>
          </button>
